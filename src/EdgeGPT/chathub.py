...
class ChatHub:
    def __init__(
        ...
        self.request = ChatHubRequest(
            conversation_signature=conversation.struct[
    ...
    async def get_conversation(
        ...
        conversation_id = quote(conversation_id or self.request.conversation_id)
        conversation_signature = quote(
                ...
                if response.get('type') == 1 and response['arguments'][0].get(
                    ...
                    if not draw:
                        ...
                        if (
                            ...
                            resp_txt_no_link = result_text + response[
                                'arguments'
                            ][0]['messages'][0].get('text', '')
                    ...
                    if draw:
                        cache = response['item']['messages'][2][
                            'adaptiveCards'
                        ][0]['body'][0]['text']
                        response['item']['messages'][2]['adaptiveCards'][0][
                            'body'
